...
class UpdateFromV10To11 extends Migration
{
    /**
     * Run the migrations.
     *
...
(101, 97, 'Delete Assets', 'assets.delete', '');";
        ...
        \Illuminate\Support\Facades\DB::table('settings')->insert([
            ...
            [
                'setting_key' => 'client_registration_required_fields',
                'setting_value' => '',
            ],
            [
        ...
        foreach (\App\Models\Loan::all() as $key) {
            ...
            if ($key->loan_status == 'open') {
                $repair_status = "disbursed";
            }
            if ($key->loan_status == 'fully_paid') {
                $repair_status = 'closed';
            }
            if ($key->loan_status == 'defaulted') {
                $repair_status = "written_off";
            }
            if ($key->loan_status == 'restructured') {
                $repair_status = "rescheduled";
            }
            ...
            \Illuminate\Support\Facades\DB::table('loans')
                ->where('id', $key->id)
                ->update(['status' => $repair_status]);
        }
        //finally update system version
        App\Models\Setting::where('setting_key', 'system_version')
            ->update(['setting_value' => '1.1']);
    }
