...
class RefreshRepaymentReversed
{
    ...
    public function handle(RepaymentReversed $event)
    {
        ...
        $payments = LoanTransaction::where('loan_id', $loan_transaction->loan_id)->where('transaction_type',
                ...
                $loan_transaction->date)->sum('credit') + $loan_transaction->credit;
                    ...
                    foreach ($loan->schedules as $schedule) {
                        if ($amount > 0) {
                            if (($schedule->principal + $schedule->fees + $schedule->penalty + $schedule->interest) < $payments) {
                                $payments = $payments - ($schedule->principal + $schedule->fees + $schedule->penalty + $schedule->interest);
